...
const Stat = ({ value, label, description }: { value: string, label: string, description: string }) => {
  ...
  return (
    <motion.div
      ...
      className="text-left"
    >
      <p className="text-4xl md:text-5xl lg:text-6xl font-bold text-foreground">{value}</p>
      <p className="mt-2 text-base md:text-md font-semibold text-foreground">{label}</p>
        ...
        <Section className="bg-secondary">
            <div className="container mx-auto px-4">
                <div className="grid md:grid-cols-2 gap-12 md:gap-16 items-center">
                    <motion.div 
                        className="relative h-[400px] md:h-[450px] w-full rounded-xl overflow-hidden"
